...
class HelpMenu(ListPageSource):
    ...
    async def write_page(self, menu, fields=[]):
        ...
        embed.set_footer(text=f"{offset:,} - {min(len_data, offset+self.per_page-1):,} of {len_data:,} friends.")
        embed.set_thumbnail(url=url["data"][0]["imageUrl"])

        field = [("User Name: ", self.userName.name, True),
                ("Display Name: ", self.userName.display_name, True),
                ("ID: ", self.userName.id, False),]
...
class Logistics(Cog):
    ...
    async def _check_friend(self, ctx, userName: Optional[str] = "roblox"):
        ...
        if username == None:
            ...
            if len(friend_ids):
                menu = CustomButtonMenuPages(source=HelpMenu(ctx, friend_ids, username),
                             timeout=120.0)
