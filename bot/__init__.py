...
class Bot(BotBase):
    ...
    async def process_commands(self, message):
        ...
        self.banlist_user = [
            user["_id"]
            for user in self.BANNED_USER_DB.find(
                {"Guild ID": message.guild.id, "Type": "user"}
            )
        ]
        self.banlist_channel = [
            channel["_id"]
            for channel in self.BANNED_USER_DB.find(
                {"Guild ID": message.guild.id, "Type": "channel"}
            )
        ]
    ...
    async def on_error(self, err, *args, **kwargs):
        if err == "on_command_error":
            ...
            embed = Embed(
                title="Errors occurred", colour=0x2F3136, timestamp=datetime.utcnow()
            )
